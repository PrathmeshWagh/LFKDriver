...
const PackageListScreen: React.FC<Props> = ({ navigation, route }) => {
    ...
    const skipHandler = async () => {
        // console.log('productDate.data', productDate.data);

        const consolidateOrderNos = productDate.data
            .filter(item => item.packing_status === 'pending')
            .map(item => item.consolidate_order_no);
        console.log('consolidateOrderNos', consolidateOrderNos);

        try {
            setLoading(true);
            const raw = {
                consolidate_order_no: consolidateOrderNos
            };
            const api: any = await postMethod('deliveryman/all-complete-packing', raw);
            console.log('api deliveryman', api.data);

            if (api.status === 200) {
                setLoading(false);
                Snackbar.show({
                    text: 'all order completed',
                    duration: Snackbar.LENGTH_SHORT,
                    textColor: 'white',
                    ...
                    <ScrollView>
                        ...
                        <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center', marginTop: 20 }}>
                            ...
                            <Pressable style={styles.skipBtn} onPress={skipHandler}>
                                <Text style={styles.skipText}> {translations.package_listing.Skip}</Text>
                            </Pressable>
                        </View>
                    </ScrollView>
