...
export default function Login({ navigation }: any) {
  ...
  const sendPlayerId = async () => {
    try {
      setLoading(true)
      const id = {
        player_id: playerId
      }
      const api: any = await postMethod(`deliveryman/get-player-id`, id)
      console.log('api player id',api.data);
      setLoading(false)
    } catch (error) {
      console.log('error in player id ',error);
      setLoading(false)
    }
